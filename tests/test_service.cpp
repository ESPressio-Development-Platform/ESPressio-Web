 ...
 * Total Memory: 60 bytes [PathValue: Capacity + 1 bytes when capacity exceeds 15-byte SSO; Headers: BucketCount * 4 bytes + N * (hash-node/link overhead + 48 bytes value); Headers: key/value: Capacity + 1 bytes when capacity exceeds 15-byte SSO; Headers: key/value: Capacity + 1 bytes when capacity exceeds 15-byte SSO]
 ...
 * Inherited Memory Total: 4 bytes [0 bytes dynamic allocation]
 * Members:
 * - Mime (std::string): 24 bytes [Capacity + 1 bytes when capacity exceeds 15-byte SSO]
 * - Reply (std::string): 24 bytes [Capacity + 1 bytes when capacity exceeds 15-byte SSO]
 * - SelectionCalls (int): 4 bytes [0 bytes dynamic allocation]
 * - HandleCalls (int): 4 bytes [0 bytes dynamic allocation]
 * Total Memory: 60 bytes [Mime: Capacity + 1 bytes when capacity exceeds 15-byte SSO; Reply: Capacity + 1 bytes when capacity exceeds 15-byte SSO]
 * Basis: ESP32/Xtensa ILP32 reference ABI (4-byte pointers/size_t); ESPressio stateful allocators/deleters included; ABI-sensitive STL/platform internals are identified explicitly.
 * Confidence: medium; compile-time sizeof on the concrete target remains authoritative for ABI-sensitive/opaque members.
 * End ESPressio Memory Audit
 */
